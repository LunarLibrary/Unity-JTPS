...
namespace JUTPS.CharacterBrain
{
    ...
    public class JUCharacterBrain : MonoBehaviour
    {
        ...
        protected virtual void Awake()
        {
            ...
            MyCamera = (MyPivotCamera != null && IsArtificialIntelligence == false && IsFPS) ? MyPivotCamera.mCamera : null;
        ...
        protected virtual void UseLeftHandItem(bool ShotInput, bool ShotDownInput)
        {
            ...
            if (HoldableItemInUseLeftHand.ContinuousUseItem)
            {
                if (ShotInput)
                {
                    if (IsRolling == false && 
                        IsDriving == false && 
                        ArmsWeightIK > 0.7f && HoldableItemInUseLeftHand.CanUseItem)
                        UseEquipedItem();
                    else HoldableItemInUseLeftHand.StopUseItem();
                }
                else HoldableItemInUseLeftHand.StopUseItem();
            }
            else
